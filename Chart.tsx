...
export default function Chart({data, yLabel, jumpToTimecode}) {
  // FIX: Correctly type chartRef to fix property access and ref assignment errors.
  ...
  const yScale = scaleLinear()
    // FIX: Provide default values to min/max to prevent errors with empty data.
  ...
  // FIX: Provide a generic type to d3.line() for better type inference.
  const lineGen = line<{time: string, value: number}>()
    // FIX: Use a non-null assertion as xScale can return undefined.
    .x((d) => xScale(d.time)!)
    .y((d) => yScale(d.value));

  useEffect(() => {
    const setSize = () => {
      // FIX: Add a null check for chartRef.current.
      if (chartRef.current) {
